...
def feature_extract(data_path, model_name, save_path, model_path):
    ...
    crackle_labels = np.array([0 for i in range(len(crackle_img))])
    wheeze_labels = np.array([1 for i in range(len(wheeze_img))])
    normal_labels = np.array([2 for i in range(len(normal_img))])
...
with tf.device("/gpu:0"):
    feature_extract(data_path, 'save_model_43.81.h5', save_path, model_path)
    K.clear_session()
