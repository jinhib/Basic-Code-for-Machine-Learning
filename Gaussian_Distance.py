...
for f_num in [10, 20, 30, 40, 50, 60, 70, 80, 90]:

    file_name = 'img_features_43.81.csv'

    dataset = np.genfromtxt(file_name, delimiter=',', encoding='UTF8', dtype='float64')
    dataset = dataset[1:, :]
    ...
    file_path = ''
    f = open(file_path + 'Gaussian_Distance_' + str(f_num) + '_' + file_name[:-4] + '.csv', 'w', encoding='utf-8',
             newline='')
    wr = csv.writer(f)
